...
struct Tweakable {
	...
	Tweakable(float v) : type(TV_FLOAT) {
		fValue = v;
	}

	Tweakable(const ds::vec2& v) : type(TV_VEC2) {
		v2Value = v;
	}

	Tweakable(const ds::vec3& v) : type(TV_VEC3) {
		v3Value = v;
	}

	Tweakable(const ds::vec4& v) : type(TV_VEC4) {
		v4Value = v;
	}

	Tweakable(const ds::Color& v) : type(TV_COLOR) {
		cValue = v;
	}
};
...
typedef std::map<TVKey, Tweakable*> TVMap;
...
FILETIME tv__getFileModTime(const char* fileName) {
...
static Tweakable* tv__addTweakable(const char* filename, size_t counter, Tweakable *twk) {
	bool found = false;
...
static Tweakable* tv__findTweakValue(const char *file, size_t counter) {
	...
	while (!feof(fp)) {
		...
		while ((ch = strstr(ch, "_T" "V("))) {
			...
			if (chend) {
				...
				strval[chend - ch] = '\0';
				DBG_LOG("TWK: %s, %d : val '%s'", srcFile, counter, strval );
				ch = chend;
				// TWK: g:\nb\breakout\src\objects\worm.cpp, 0 : val '1.0f, 1.0f'
				// TWK: g:\nb\breakout\src\objects\worm.cpp, 1 : val '40.0f'
				// Apply the tweaked value (if found)
				Tweakable *tv = tv__findTweakValue(srcFile, counter);
				if (tv)	{
					if (tv->type == Tweakable::TV_INT) {
						tv->iValue = atoi(strval);
					}
					else if (tv->type == Tweakable::TV_FLOAT) {
						tv->fValue = atof(strval);
					}
					else if (tv->type == Tweakable::TV_VEC2) {
						float f[2];
						tv__extract(strval, f, 2);
						tv->v2Value = ds::vec2(f[0],f[1]);
					}
					else if (tv->type == Tweakable::TV_VEC3) {
						float f[3];
						tv__extract(strval, f, 3);
						tv->v3Value = ds::vec3(f[0], f[1], f[2]);
					}
					else if (tv->type == Tweakable::TV_VEC4) {
						float f[4];
						tv__extract(strval, f, 4);
						tv->v4Value = ds::vec4(f[0], f[1], f[2], f[3]);
					}
					else if (tv->type == Tweakable::TV_COLOR) {
						float f[4];
						tv__extract(strval, f, 4);
...
float _TweakValue(const char* file, size_t counter, float orgValue) {
	Tweakable* tv = tv__findTweakValue(file, counter);
	if (tv) {
		return tv->fValue;
	}
	else {
		Tweakable *twk = new Tweakable(orgValue);
